...
class VibeEngine:
    ...
    def analyze(self, energy_ratio):
        ...
        if now - self.last_switch > 10.0:
            if density > 4.0:
                self.current_vibe = "RAGE"
            elif density > 1.0:
                self.current_vibe = "PARTY"
            else:
                self.current_vibe = "JAZZ"
...
class AudioReactive:
    ...
    def process_audio(self, indata):
        if self.override_mode or self.static_mode:
            return
        ...
        if rms < SILENCE_THRESHOLD:
            self.target_brightness, self.target_hue, self.target_sat = (
                MIN_BRIGHTNESS,
                READING_HUE,
                READING_SAT,
            )
        ...
        if mode in ["JAZZ", "PARTY", "RAGE", "DYNAMIC"]:
            mode = self.vibe.analyze(bass_ratio)

        if mode == "KCD2":
            e_rain = np.sum(fft_data[(freqs > 2000) & (freqs < 8000)])
            detected_wet = e_rain > (self.avg_bass * 1.5) and e_rain > 0.5
            if detected_wet:
                self.weather_accumulator = min(1.0, self.weather_accumulator + 0.05)
            else:
                self.weather_accumulator = max(0.0, self.weather_accumulator - 0.02)
            ...
            if e_steel > (self.avg_bass * 6.0) and e_steel > 0.6:
                ...
                self.target_hue, self.target_sat, self.target_brightness = (
                    0.05,
                    0.8,
                    MIN_BRIGHTNESS + 0.3,
                )
                ...
                self.target_hue, self.target_sat = (
                    (0.55, 0.3) if is_wet else (0.08, 0.9)
                )
                thunder_mod = (
                    (np.sin(time.time() * 15) * 0.1)
                    if e_thunder > (self.avg_bass * 2.0)
                    else 0
                )
                self.target_brightness = np.clip(
                    MIN_BRIGHTNESS
                    + thunder_mod
                    + (random.uniform(-0.03, 0.03) if is_wet else 0),
                    0,
                    1.0,
                )
            return
        ...
        SCAPES = {
            "WAR_ZONE": {
                "ambient": (0.33, 1.0),
                "events": [
                    (2000, 20000, 3.0, 0.0, 0.0, 1.0),
                    (0, 200, 3.0, 0.03, 1.0, 0.8),
                ],
            }
        }
        if mode in SCAPES:
        ...
        self.target_brightness = MIN_BRIGHTNESS + (
            np.clip((bass_ratio - 0.5) / 2.0, 0, 1)
            * (MAX_AUDIO_BRIGHTNESS - MIN_BRIGHTNESS)
        )
        if mode in ["WARM_COLD", "VAPORWAVE", "ROCK", "JAZZ", "TECHNO", "LOFI"]:
            p = PALETTES[mode]
            idx = (
                int(np.clip(bass_ratio * 0.5 * 2.99, 0, 2))
                if mode != "WARM_COLD"
                else int(np.clip(bass_ratio, 0, 2))
            )
            ...
            if np.any((freqs > 250) & (freqs < 4000)):
                ...
                centroid = np.sum(freqs[(freqs > 250) & (freqs < 4000)] * mid_fft) / (
                    np.sum(mid_fft) + 1e-6
                )
                self.target_hue = np.clip(
                    (np.log10(max(centroid, 250)) - np.log10(250))
                    / (np.log10(4000) - np.log10(250)),
                    0,
                    1,
                )
    ...
    async def led_control_loop(self):
        while self.running:
            ...
            if self.led:
                ...
                if self.current_brightness <= (
                    MIN_BRIGHTNESS + 0.01
                ) and self.target_brightness > (MIN_BRIGHTNESS + 0.05):
                    self.current_hue = self.target_hue
                self.current_brightness = (self.current_brightness * 0.8) + (
    ...
    async def handle_ping(self, color_name):
        async with self.ping_lock:
            ...
            if self.led:
                for _ in range(2):
                    await self.led.set_rgb((255, 255, 255))
                    await asyncio.sleep(0.05)
                    await self.led.set_rgb((0, 0, 0))
                    await asyncio.sleep(0.1)
                for i in range(0, 101, 20):
                    f = i / 100.0
                    await self.led.set_rgb(
    ...
    async def handle_client(self, reader, writer):
        ...
        if msg.startswith("PING"):
            p = msg.split(" ")
            color = p[1] if len(p) > 1 else "green"
            asyncio.create_task(self.handle_ping(color))
        elif msg.startswith("MODE"):
            p = msg.split(" ")
            m = p[1].upper() if len(p) > 1 else "JAZZ"
            if m == "STATIC":
                self.static_mode = True
            else:
                self.static_mode = False
                self.vibe.current_vibe = m
        elif msg.startswith("SET"):
            p = msg.split(" ")
            if len(p) >= 4:
                try:
                    READING_HUE, READING_SAT, MIN_BRIGHTNESS = (
                        float(p[1]),
                        float(p[2]),
                        float(p[3]),
                    )
        ...
        while self.running:
            try:
                ...
                if device:
                    ...
                    with sd.InputStream(
                        callback=lambda d, f, t, s: self.process_audio(d),
                        device=target_id,
                        channels=1,
